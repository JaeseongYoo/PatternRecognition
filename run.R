rm(list = ls())
setwd("E:/Dropbox/JaeseongYoo/패턴인식/Term Project/R Codes")


##### 라이브러리 및 필요한 코드 import
...
### 데이터를 Topology로부터 생성할 경우
nodes = 7
temp = make_Rhombus(nodes)   # 여기서 topology 형태를 지정해준다. Collapse, Diamond, Line, PseudoLoop, Star, Rhombus
arcs = temp$arcs
...
### 데이터를 Real Model로부터 생성할 경우
# source("real_asia.R")			## Asia 데이터
# source("real_insurance.R")		## Insurance 데이터
# source("real_alarm.R")		## Alarm 데이터
# source("real_hailfinder.R")	## HailFInder 데이터
